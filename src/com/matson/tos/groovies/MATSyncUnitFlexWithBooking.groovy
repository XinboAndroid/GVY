...

{
    ...
    {
        LOGGER.info("Execution Started MATSyncUnitFlexWithBooking");
        /* get the event */
        api = new GroovyApi();
        ...
        Booking ThisBooking = getBookingFromEvent(inEvent);
        ...
        LOGGER.info("Updating srv company : 989898");
        update(inEvent, ThisBooking);
    }

   /* private Booking getBookingFromEvent(GroovyEvent inEvent) {
        def gvyEquiObj = api.getGroovyClassInstance("GvyCmisSrvMsgProcessor");
        return gvyEquiObj.getBookingFromEvent(inEvent.getEvent());
    }*/


    public void update(GroovyEvent inEvent, Booking thisBooking) {
        Event thisEvent = inEvent.getEvent();
        LOGGER.info("inside update srv company : " + thisBooking.getEqboNbr());
        if (thisEvent == null) {
            return;
        }
        ...
        LOGGER.info("Start Event ---:" + thisEvent.getEventTypeId() + " on Unit :" + unit.getUnitId() + " ---:")
        ...
        try {
            if (unit != null) {
                ...
                LOGGER.info("56565656Loaded unit from DB : " + unit.getUnitId());
                ...
                LOGGER.info("ueEquipmentState :equipFlex01(getEqsFlexString01) Value---:" + equipFlex01)
                String serviceId = setEqCntrSvr(inEvent, thisBooking);
                LOGGER.info("ueEquipmentState :equipFlex01(getEqsFlexString01) Value1---:" + equipFlex01)
                if (equipFlex01 != null) {
                    def newEquipFlex01 = ueEquipmentState != null ? ueEquipmentState.getEqsFlexString01() : '';
                    LOGGER.info("ueEquipmentState :newEquipFlex01(getEqsFlexString01) Value-11--:" + newEquipFlex01);
                    if (newEquipFlex01 != null && "MAT".equalsIgnoreCase(newEquipFlex01) && !"MAT".equalsIgnoreCase(equipFlex01)) {
                        isAlwaysSendIGT = true;
                        LOGGER.info("isAlwaysSendIGT true")
                    }
                }
    ...
    public String setEqCntrSvr(GroovyEvent event, Booking thisBooking) {
        LOGGER.info("Inside setEqCntrSvr");
        //def gvyPropUpdtObj = api.getGroovyClassInstance("GvyCmisSrvMsgProcessor");
        String EqContainerService = getNewEqFlexString01(event, thisBooking);
        LOGGER.info("The new Equipment Contianer Service is " + EqContainerService);
        Unit unit = event.getEntity();
        try {
            UnitEquipment unitEquipment = unit.getUnitPrimaryUe();
            EquipmentState ueEquipmentState = unitEquipment.getUeEquipmentState();
            ueEquipmentState.setEqsFlexString01(EqContainerService);
            LOGGER.info("Upgraded setEqsFlexString01 to " + EqContainerService);
        } catch (Exception ex) {
            LOGGER.error("Exception when Updating Unit ueEquipmentState.setEqsFlexString01 with "
                    + EqContainerService, ex);
        }
        return EqContainerService;
    }

    public Booking getBookingFromEvent(GroovyEvent inEvent) {
        ...
        Unit ThisUnit = (Unit) ThisEvent.getEntity();
    ...
    public String getNewEqFlexString01(GroovyEvent inEvent, Booking thisBooking) {
        String srvId = "MAT";
