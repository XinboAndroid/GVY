...
public class GvyCmisEventUnitStrip
{
    ...
    public String processUnitStrip(Object unit, String eventNotes, Object gvyEventObj)
    {
        ...
        try
        {
            ...
            {
                if(doer != null && doer.contains('ACETS')){
                    //actionAttributes = actionAttributes.replace("lastAction='null'","lastAction='AMTX'") //A7
                    //actionAttributes = actionAttributes.replace("action='null'","action='AMTX'")
                }else{
                    // actionAttributes = actionAttributes.replace("lastAction='null'","lastAction='MTX'")  //A7
                    // actionAttributes = actionAttributes.replace("action='null'","action='MTX'")
                    //println(" -------------------------- THIS MTX MSG HAS BEEN SUPPRESSED --------------------------- ")
                }
            }
    ...
    public void depndMtxActionForSn4Q(Object event,Object unit, Object gvyBaseClass){
        try{
            ...
            //DTD Messages
            if('HON'.equals(dischPort) && misc3.length() > 6){
